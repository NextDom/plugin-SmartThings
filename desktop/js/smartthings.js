 ...
 */
 ...
 $('#bt_syncSmartThings').on('click', function () {
	$.ajax({ // fonction permettant de faire de l'ajax
 ...
 $('#bt_healthSmartThings').on('click', function () {
	$('#md_modal').dialog({title: "{{Santé Home Connect}}"});
	$('#md_modal').load('index.php?v=d&plugin=smartthings&modal=health').dialog('open');
});
 ...
 $('.eqLogicAttr[data-l1key=configuration][data-l2key=type]').on('change',function(){
	...
	$('#img_device').attr("src", 'plugins/smartthings/core/config/images/'+$(this).value()+'.png');
});
...
function addCmdToTable(_cmd) {
  ...
  tr += '</td>';
  tr += '<td>';
  tr += '<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isVisible" checked/>{{Afficher}}</label><br/></span> ';
